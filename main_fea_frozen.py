...
def train():
    for epoch_idx in range(start_epoch, args.epochs):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            if (batch_idx%10==0)or(batch_idx==len(TrainImgLoader)-1):
        ...
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            del scalar_outputs, image_outputs
            if (batch_idx%1==0)or(batch_idx==len(TestImgLoader)-1):
...
def test_sample(batch_idx,sample, compute_metrics=True):
    ...
    #driving test
    # test_gt_path='/data/yyx/GwcNet-master/checkpoints/sceneflow_monkaa/gwcnet-gc/sceneflow_test_gt/'
    # test_pred1_path='/data/yyx/GwcNet-master/checkpoints/sceneflow_monkaa/gwcnet-gc/sceneflow_test_pred/'
    ...
    # im_pred1 = np.array(disp_ests[0][0,:, :].cpu()*2, dtype=np.uint16)
    # cv.imwrite(join(test_pred1_path, "sceneflow-%d.png" % batch_idx),im_pred1)

    #kitti test
    test_gt_path='/data/yyx/GwcNet-master/checkpoints/kitti/kitti_test_gt/'
    test_pred1_path='/data/yyx/GwcNet-master/checkpoints/kitti/kitti_test_pred/'
        
    _, H, W = disp_ests[0].shape
    gt=torch.zeros((H,W))
    im_pred1 = torch.zeros(( H, W))

    gt = np.array(disp_gt[0,:, :].cpu()*256, dtype=np.uint16)
    cv.imwrite(join(test_gt_path, "kitti-gt-%d.png" % batch_idx),gt)
    im_pred1 = np.array(disp_ests[0][0,:, :].cpu()*256, dtype=np.uint16)
    cv.imwrite(join(test_pred1_path, "kitti-%d.png" % batch_idx),im_pred1)
