...
parser.add_argument('--train_mono', action='store_true', help='train the model')
parser.add_argument('--train_bio', action='store_true', help='train the model')
...
logger = SummaryWriter(comment='depth_mono')
...
def train():
    for epoch_idx in range(start_epoch, args.epochs):
        ...
        # training
        if args.train:
            assert args.train_mono or args.train_bio
            nannum=0
            mono_nannum=0
...
def train_sample(sample, compute_metrics=False):
    ...
    # print(imgL.shape, disp_gt.shape)
    imgL = imgL.cuda()
    imgR = imgR.cuda()
    ...
    if args.train_mono:
        ...
        if args.dataset == "sceneflow":
            depth_mask = (depth_gt > 0.1) & (depth_gt < args.mono_max_depth)
        else :
            depth_mask = depth_gt > 1.0
        # print(mono_final_depth.shape,depth_gt.shape)
